...
const {
  mode
} = __webpack_require__(/*! ../../webpack.config */ "./webpack.config.js");
...
__webpack_require__.r(__webpack_exports__);
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, avticeClass) {
  // --------------------------------------- Tabs
  ...
  function hideTabContent() {
    ...
    tabs.forEach(item => {
      item.classList.remove(avticeClass);
    });
  }
  ...
  function showTabContent() {
    ...
    tabs[i].classList.add(avticeClass);
  }
  hideTabContent();
  showTabContent();
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    const t = Date.parse(endtime) - Date.parse(new Date()),
      //Date.parse Превращает наш стринг в милисекунды
      days = Math.floor(t / (1000 * 60 * 60 * 24)),
      hours = Math.floor(t / (1000 * 60 * 60) % 24),
      // % возвращает остаток от деления на число (24)
  ...
  function getZero(num) {
    // эта функция подставляет ноль если число из одной цифры, например 9 = 09
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2023-08-02');
});
}();
/******/ })()
;
